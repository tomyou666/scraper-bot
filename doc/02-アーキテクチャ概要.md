...
- 起動順:
    ...
    2. `Config` 構築・バリデーション
    3. `core.NewHost` で Host を構築しカーネル生成
    4. `Kernel.Init` で P2 → P3 → P5 → P6 → P7 → P8 の順に `Init(ctx, host)`
    5. ユースケース実行（パイプライン or クローラ）。クロール時は `robots.NewCache(kernel.Fetcher())`
    6. 終了シグナル受信時、登録の逆順で `Close(ctx)`
- 失敗ポリシー:
